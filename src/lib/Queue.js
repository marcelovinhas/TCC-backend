...
class Queue {
  ...
  init() {
    ...
    jobs.forEach(({ key, handle }) => {
      this.queues[key] = {
        bee: new Bee(key, {
          // key como primeiro parâmetro, segundo parâmetro passa as configurações
          redis: redisConfig,
        }),
  ...
  add(queue, job) {
    // método para adicionar jobs dentro de cada fila
  ...
  handleFailure(job, error) {
    // método para mostrar erros na fila
    console.log(`FALHA NA QUEUE: Queue ${job.queue.name}: FAILED`, error);
  }
}
